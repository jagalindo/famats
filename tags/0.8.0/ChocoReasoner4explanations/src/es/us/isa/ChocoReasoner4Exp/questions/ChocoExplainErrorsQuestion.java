...
import es.us.isa.ChocoReasoner4Exp.ChocoResult;
...
public class ChocoExplainErrorsQuestion extends ChocoQuestion implements ExplainErrorsQuestion{
	...
	public PerformanceResult answer(Reasoner r){
		...
		while (itE.hasNext()){
			//crear una lista de constraints, que impondremos segun las observaciones
			...
			//la funcion a maximizar sera el sumatorio de las vbles reificadas
			IntegerVariable[] reifieds = new IntegerVariable[chReasoner.getReifiedVars().size()];
			Iterator<IntegerVariable> it=chReasoner.getReifiedVars().values().iterator();
			int i=0;
			while(it.hasNext()){
				reifieds[i]=it.next();
				i++;
			}
			IntegerVariable suma = Choco.makeIntVar("suma", 0, chReasoner.getRelations().size());
