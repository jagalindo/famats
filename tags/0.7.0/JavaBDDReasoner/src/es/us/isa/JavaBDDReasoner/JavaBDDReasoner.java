...
public class JavaBDDReasoner extends FeatureModelReasoner {
	...
	private BDD bdd;						// BDD
	private Map<String,GenericFeature> featuresMap;		// Features Map that links vars and features
	...
	public void reset() {
		...
		factory=JFactory.init(1000000,10000);		// This can be optimized taking into account the size of the problem
		numvar=1;
	}
	...
	//Debe recoger como parametro GenericFeature?
...
/*	*//**
	...
	public void addSet(GenericRelation rel, GenericFeature parent,
		...
		Iterator iter = cardinalities.iterator();
		...
		} else
		{
			...
			for (int i=0;i<childrenList.size();i++)
			{
				...
				if (bdd_alternative==null)
					bdd_alternative=childrenList.get(i).apply(tmp_alternative,BDDFactory.biimp);
				else
					bdd_alternative.andWith(childrenList.get(i).apply(tmp_alternative,BDDFactory.biimp));
			}
